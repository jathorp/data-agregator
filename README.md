...
| **NFR-06** | **Security**               | Communication must be encrypted-in-transit (TLS 1.2+) at all stages. Data must be encrypted-at-rest in S3 and SQS. **Access credentials for the NiFi endpoint (e.g., API key or bearer token) will be managed by AWS Secrets Manager** with a defined rotation policy. |
...
## 4\. Proposed Architecture (v5.0 - As-Built Design)
...
flowchart TD
  ...
  Lambda        -->|"8 - POSTs compressed batch (HTTPS via private network)"| NiFi
...
### 4.3. Design Considerations & Risk Mitigation
  ...
  * **Secure Delivery:** **The compressed data archive will be sent as the body of an `HTTP POST` request to the configured NiFi endpoint. The request will include necessary headers, such as `Content-Type: application/gzip` and an `Authorization` header containing the credential retrieved from Secrets Manager.**
  ...
  * **Secrets Management:** Credentials for the **on-premise NiFi endpoint** are retrieved from AWS Secrets Manager. The Lambda caches these credentials in-memory for 5 minutes to improve performance.
...
| **2. Dev & Test**  | Lambda Logic & Unit Testing      | Develop idempotent **Python** code for the Lambda. The code is structured into testable modules (`core.py`, `clients.py`). **The `clients.py` module will use the `requests` library to handle the HTTP POST to NiFi.** A comprehensive `pytest` suite will use `moto` to mock AWS services and **`requests-mock` to simulate the NiFi endpoint response.** |
| **3. Integration** | End-to-End & Load Testing        | Deploy to a staging environment. Conduct load tests simulating baseline (10 files/sec) and burst loads (100 files/sec). **Crucially, test the outage scenario** by blocking connectivity to a **mock HTTP endpoint** and verifying that SQS buffers correctly and the system recovers.                         |
