...
def test_process_and_stage_batch_happy_path(mock_create_bundle, mock_lambda_context):
    ...
    mock_hash = "fake_hash"
    test_records: list[S3EventRecord] = [{"s3": {"bucket": {"name": "b"}, "object": {"key": "f1.txt", "size": 10}}}]

    # Update the mock to return the new 3-item tuple
    mock_create_bundle.return_value.__enter__.return_value = (mock_bundle_file, mock_hash, test_records)
    ...
    # ASSERT
    mock_create_bundle.assert_called_once_with(mock_s3_client, test_records, mock_lambda_context)
    mock_s3_client.upload_gzipped_bundle.assert_called_once_with(
        bucket="dist-bucket", key="bundle.tar.gz", file_obj=mock_bundle_file, content_hash=mock_hash
    )
...
def test_create_tar_gz_bundle_stream_happy_path(mock_lambda_context):
    ...
    mock_s3_client.get_file_content_stream.side_effect = [io.BytesIO(file1), io.BytesIO(file2)]
    records: list[S3EventRecord] = [
        {"s3": {"bucket": {"name": "b"}, "object": {"key": "f1.txt", "size": len(file1)}}},
        {"s3": {"bucket": {"name": "b"}, "object": {"key": "d/f2.log", "size": len(file2)}}},
    ]

    # ACT
    # Unpack all three return values
    with create_tar_gz_bundle_stream(mock_s3_client, records, mock_lambda_context) as (f, r_hash, p_records):
...
def test_create_tar_gz_bundle_stream_stops_gracefully_on_timeout(mock_lambda_context):
    ...
    # ACT
    with create_tar_gz_bundle_stream(mock_s3_client, records, mock_lambda_context) as (_, _, processed_records):
...
def test_create_tar_gz_bundle_stream_stops_gracefully_on_disk_limit(mock_lambda_context):
    ...
    records: list[S3EventRecord] = [
        {"s3": {"object": {"key": "f1.txt", "size": file1_size}, "bucket": {"name": "b"}}},
...
def test_create_tar_gz_bundle_stream_skips_mismatched_size_file(mock_lambda_context):
    ...
    # ARRANGE
    mock_s3_client = MagicMock()
    mock_s3_client.get_file_content_stream.return_value = io.BytesIO(b"actually 10 bytes")
    records: list[S3EventRecord] = [{"s3": {"object": {"key": "bad.txt", "size": 100}, "bucket": {"name": "b"}}}]

    # ACT
    with create_tar_gz_bundle_stream(mock_s3_client, records, mock_lambda_context) as (f, _, p_records):
        bundle_content = f.read()
